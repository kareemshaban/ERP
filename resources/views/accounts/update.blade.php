...
<main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <!-- Navbar -->
    @include('layouts.nav' , ['page_title' => __('main.main.accounts_list'). ' / '. __('main.update_account')])
    <!-- End Navbar -->
    <div class="modal-body" id="paymentBody">
        <form   method="POST" action="{{ route('updateProduct' , $account -> id) }}">
            @csrf
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="account_type" name="type">
                                    <option value="1" @if($account->type == 1) selected @endif>{{__('main.Root')}}</option>
                                    <option value="2" @if($account->type == 2) selected @endif>{{__('main.General')}}</option>
                                    <option value="3" @if($account->type == 3) selected @endif>{{__('main.Branch')}}</option>
                                    <option value="4" @if($account->type == 4) selected @endif>{{__('main.Branch_Ledger')}}</option>
                                </select>
                                @error('type')
                                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('brand') is-invalid @enderror" id="parent_id" name="parent_id" disabled>
                                    <option value="0"@if($account->parent_id == 0) selected @endif></option>
                                    @foreach($accounts as $brand)
                                        <option value="{{$brand->id}}"  @if($account->parent_id == $brand->id) selected @endif>{{$brand->name}}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        <div class="col-12 " >
                            <div class="form-group">
                                ...
                                <input type="text"  id="level" name="level" readonly  value="{{$account->level}}"
                                       class="form-control @error('name') is-invalid @enderror"
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="list" name="list">
                                    <option value="1"  @if($account->type == 1) selected @endif>{{__('main.Assets')}}</option>
                                    <option value="2" @if($account->type == 2) selected @endif>{{__('main.Discounts')}}</option>
                                    <option value="3" @if($account->type == 3) selected @endif>{{__('main.Income')}}</option>
                                    <option value="4" @if($account->type == 4) selected @endif>{{__('main.Expenses')}}</option>
                                </select>
                                @error('type')
                                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="department" name="department">
                                    <option value="1" @if($account->type == 1) selected @endif>{{__('main.Balance_Sheet')}}</option>
                                    <option value="2" @if($account->type == 2) selected @endif>{{__('main.Incoming_List')}}</option>
                                </select>
                                @error('type')
                                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="side" name="side">
                                    <option value="1" @if($account->type == 1) selected @endif>{{__('main.Credit')}}</option>
                                    <option value="2" @if($account->type == 2) selected @endif>{{__('main.Debit')}}</option>
                                </select>
                                @error('type')
                                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
